...
float DallasTemperature::calculateTemperature(uint8_t* deviceAddress, uint8_t* scratchPad)
{
  ...
  switch (deviceAddress[0])
  {
    ...
    case DS18S20MODEL:
      ...
      the temperature, COUNT REMAIN and COUNT PER пїЅC registers in the
      scratchpad. Note that the COUNT PER пїЅC register is hard-wired to 16
      (10h). After reading the scratchpad, the TEMP_READ value is obtained
      by truncating the 0.5пїЅC bit (bit 0) from the temperature data. The
