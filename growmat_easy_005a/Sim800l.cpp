...
String Sim800l::getOperator(){
   ...
   if ( (_buffer.indexOf("+COPS: ") )!=-1 ) {
	   if(_buffer.lastIndexOf('"') == -1) {
		   return String(F("CHYBA GSM!"));
	   }
       return _buffer.substring(_buffer.indexOf("+COPS: ")+7+5, _buffer.lastIndexOf('"'));
   }
   return String();
}

String Sim800l::getSignal(){
   SIM.print (F("AT+CSQ\r\n"));
   _buffer=_readSerial();
   if ( (_buffer.indexOf("+CSQ: ") )!=-1 )
	   return _buffer.substring(_buffer.indexOf("+CSQ: ")+6, _buffer.lastIndexOf(','));
   return String();
}

String Sim800l::getReg(){
   SIM.print (F("AT+CREG?\r\n"));
   _buffer=_readSerial();
   if ( (_buffer.indexOf("+CREG: ") )!=-1 )
	   return _buffer.substring(_buffer.indexOf("+CREG: ")+9, _buffer.length());// _buffer.lastIndexOf(','));
   return String();
}
...
void Sim800l::enableRTC()  {
	SIM.print(F("AT+CLTS=1;&W\r\n"));
	while (_readSerial().indexOf("OK")==-1 ){
		SIM.print(F("AT\r\n"));
	}
	//wdt_reset();
...
String Sim800l::RTCtime(int *day,int *month, int *year,int *hour,int *minute, int *second){
  SIM.print(F("at+cclk?\r\n"));
